...
<slider>
    <div slot-scope="{ navigate, showLeft, showRight, currentSlide, slidesTotal }">
        <div {{ $attributes->twMerge('relative') }}>
            <div ref="slider" {{ $items->attributes->twMerge('*:w-1/2 *:md:w-1/3 *:xl:w-1/4 *:px-5 *:shrink-0 *:snap-start -mx-5 flex snap-x snap-mandatory overflow-x-auto scrollbar-hide') }}>
                @slotdefault('items')
