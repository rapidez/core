        ...
        @else
            <div v-for="(superAttribute, superAttributeId) in config.product.super_attributes">
                ...
                <x-rapidez::select label="" v-bind:id="'super_attribute_'+superAttributeId" v-bind:name="superAttributeId" v-model="options[superAttributeId]" class="mb-3 block w-64">
                    ...
                    <option v-for="option in config.product[superAttribute.code]" v-text="option.label" :value="option.value" :disabled="disabledOptions[superAttribute.code].includes(option.value)" />
                </x-rapidez::select>
            </div>
            <div class="mt-5 flex flex-wrap items-center gap-3">
                ...
                <x-rapidez::select name="qty" label="Quantity" v-bind:value="qty" v-on:input="changeQty" class="w-auto" labelClass="flex items-center sr-only" wrapperClass="flex">
