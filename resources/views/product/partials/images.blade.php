    ...
    <images v-cloak>
        <div class="absolute inset-0" slot-scope="{ images, active, zoomed, toggleZoom, change }">
            <div class="relative" v-if="images.length">
                ...
                <button class="top-1/2 -translate-y-1/2 left-3 {{ config('rapidez.frontend.z-indexes.lightbox') }}" :class="zoomed ? 'fixed' : 'absolute'" v-if="active" v-on:click="change(active-1)" aria-label="@lang('Prev')">
                    <x-heroicon-s-chevron-left class="h-12 w-12"/>
                </button>

                <button class="top-1/2 -translate-y-1/2 right-3 {{ config('rapidez.frontend.z-indexes.lightbox') }}" :class="zoomed ? 'fixed' : 'absolute'" v-if="active != images.length-1" v-on:click="change(active+1)" aria-label="@lang('Next')">
                    <x-heroicon-s-chevron-right class="h-12 w-12"/>
                </button>
            </div>
            <x-rapidez::no-image v-else class="rounded h-96"/>

            <div v-if="images.length > 1" class="flex" :class="zoomed ? 'fixed bottom-0 left-3 {{ config('rapidez.frontend.z-indexes.lightbox') }}' : ' flex-wrap mt-3'">
                <a
                    href="#"
            ...
            <div v-if="zoomed" class="fixed top-3 right-3 pointer-events-none {{ config('rapidez.frontend.z-indexes.lightbox') }}">
