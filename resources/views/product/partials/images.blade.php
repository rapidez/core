    ...
    <images v-cloak>
        <div slot-scope="{ images, active, zoomed, toggleZoom, change }">
            <div class="relative" v-if="images.length">
                <a :href="config.media_url + '/catalog/product' + images[active]" class="flex items-center justify-center" :class="zoomed ? 'fixed inset-0 bg-white {{ config('rapidez.z-indexes.lightbox') }} cursor-[zoom-out]' : 'border rounded p-5 h-[440px]'" v-on:click.prevent="toggleZoom">
                    <img v-if="!zoomed" :src="'/storage/resizes/400/catalog/product' + images[active] + '.webp'" alt="{{ $product->name }}" class="m-auto max-h-[400px] w-full object-contain" width="400" height="400" />
                    <img v-else :src="config.media_url + '/catalog/product' + images[active]" alt="{{ $product->name }}" class="max-h-full max-w-full" loading="lazy" />
                </a>
            ...
            <div v-if="images.length > 1" class="flex" :class="zoomed ? 'fixed bottom-0 left-3 {{ config('rapidez.z-indexes.lightbox') }}' : ' flex-wrap mt-3'">
                <a href="#" v-for="(image, imageId) in images" class="mr-3 mb-3 flex h-[100px] w-[100px] items-center justify-center rounded border bg-white p-2" :class="active == imageId ? 'border-primary' : ''" @click.prevent="change(imageId)">
