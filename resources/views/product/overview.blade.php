...
@section('content')
    ...
    <product v-cloak images="{{ $product->images }}">
        <div class="flex flex-col sm:flex-row mb-5" slot-scope="{ changeAttribute, productImages }">
            <div class="sm:w-2/3 sm:mr-5">
                <div v-cloak v-if="!productImages" class="{{ $product->images->count() == 1 ? '' : 'grid gap-3 grid-cols-2' }}">
                ...
                <div v-else :class="productImages.length === 1 ? '' : 'grid gap-3 grid-cols-2'">
