...
@section('content')
    ...
    <graphql v-cloak query='@include("rapidez::cart.queries.cart")'>
        <div v-if="data && data.cart && data.cart.items && Object.keys(data.cart.items).length" slot-scope="{ data, runQuery, hasItems }">
            ...
            <graphql-mutation :updatecart="true" :changes="{cart_id: data.cart.id, cart_items: {cart_item_id: Number(item.id), quantity: Number(item.quantity)}}" query='@include("rapidez::cart.queries.changeQty")'>
                <div class="w-2/6 sm:w-1/6 lg:w-1/12" slot-scope="{ changes, mutate, mutated }">
                    <div class="inline-flex">
                        ...
                        />
                        <button
                            v-on:click="mutate().then(runQuery());"
            ...
            <div class="w-2/6 sm:w-1/6 lg:w-1/12 flex justify-end text-right">
                ...
                <graphql-mutation :updatecart="true" :changes="{ cart_id: data.cart.id, cart_item_id: Number(item.id) }" query="@include('rapidez::cart.queries.removeItemFromCart')">
                    <button :disabled="$root.loading" slot-scope="{changes, mutate}" href="#" v-on:click="mutate().then(runQuery)" class="ml-2" title="@lang('Remove')" :dusk="'item-delete-'+index">
