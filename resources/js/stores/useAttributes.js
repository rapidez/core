...
export const refresh = async function () {
    ...
    try {
        isRefreshing = true;
        var response = await axios.get('/api/attributes').finally(() => {isRefreshing = false;})
    } catch (error) {
        console.error(error)
...
export const attributes = computedAsync(async () => {
    if (Object.keys(attributesStorage.value).length === 0) {
        await refresh();
    }

    return attributesStorage;
}, attributesStorage.value, { lazy: true, shallow: false })
