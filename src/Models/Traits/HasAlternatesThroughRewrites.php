...
{
    /** @return Attribute<Collection<string, string>, null> */
    protected function alternates(): Attribute
    {
        return Attribute::make(
            get: fn () => $this
                ...
                ->whereHas('store', fn (Builder $query): Builder => $query->where('store.group_id', config('rapidez.group')))
                ->whereNot('store_id', config('rapidez.store'))
                ->pluck('request_path', 'store_id')
                ->mapWithKeys(function (string $url, int $storeId): array {
                    return Rapidez::withStore($storeId, function () use ($url): array {
